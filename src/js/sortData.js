...
export class SortData {
  ...
  render() {
    ...
    for (let j = 0; j < this.sortedElementsCount; j++) {
      const sortItem = new SortItem({value: this.getRandomNumber(1, 99)});
  ...
  buttonClickHandler = () => {
    if (this.sortArray[this.sortIndex].value < this.sortArray[this.sortIndex - 1].value) {
      this.infoField.innerHTML = `Меняем местами элементы <span>${this.sortArray[this.sortIndex - 1].value}</span> и <span>${this.sortArray[this.sortIndex].value}</span>`;
      ...
      this.infoField.innerHTML = (this.sortArray[this.sortIndex - 1].value === this.sortArray[this.sortIndex].value)
        ? `Элементы <span>${this.sortArray[this.sortIndex - 1].value}</span> и <span>${this.sortArray[this.sortIndex].value}</span> равны`
        : `Элемент <span>${this.sortArray[this.sortIndex - 1].value}</span> меньше элемента <span>${this.sortArray[this.sortIndex].value}</span>. Действия не требуются`;
    }
